...
class EmbeddingStore(nn.Module):
    ...
    def get_embeddings_from_workout_ids(
        ...
        if self.subject_embeddings is not None:
            # Indices must live on the same device as the embedding weights (GPU-safe).
            device = self.subject_embeddings.weight.device
            index_list = []
            for wid in workout_ids:
                key = wid.item() if torch.is_tensor(wid) else wid
                index_list.append(self.workout_id_to_embedding_index[key])
            subject_indices = torch.as_tensor(index_list, dtype=torch.long, device=device)
...
class ODEModel(nn.Module):
    ...
    def forecast_batch(
        ...
        # Integration grid must match x0's device (original code always used CPU).
        times = torch.arange(0, 1 + times.shape[1], device=x0.device, dtype=x0.dtype)
        xs = odeint(
            self,
            x0,
            times,
